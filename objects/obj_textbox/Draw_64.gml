if global.in_dialogue{
	...
	text_width_limit = ((sprite_get_width(spr_textbox)- (speaker_sprite[j] != noone)*sprite_get_width(speaker_sprite[j])))*minimap_scale - x_border*2;
	draw_text_ext(x*minimap_scale + x_border + text_offset, y*minimap_scale + y_offset + y_border, display_text,  line_sep, text_width_limit);
	//options
	if display_text == text[j] && j == array_length(text)-1{
		...
		for ( var op = 0; op< option_number; op++){
			//the option text
			draw_set_colour(c_white);
			draw_text(x*minimap_scale + x_border + text_offset + option_x_offset + op* _op_space /option_number, y*minimap_scale + y_offset + option_y_offset, option[op]);
			//the arrow
			if option_pos == op{
	...
	if timer <= 0 && i <= string_length(text[j]){
		...
		timer = 1;
		if accept_key||skip_key{
			if string_length(display_text) > 3{
			display_text = text[j];
