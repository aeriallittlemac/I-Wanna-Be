if global.in_inventory{
	...
	draw_sprite_ext(spr_inventory_box, -1, 0, 0, minimap_scale, minimap_scale, 0, c_white, 1 );
	...
	//options
		item_pos += keyboard_check_pressed(vk_up) - keyboard_check_pressed(vk_down);
		...
		for ( var op = 0; op< array_length(inventory); op++){
			//the option text
			draw_set_colour(c_white);
			draw_text_transformed(x_border + text_offset + option_x_offset, option_y_offset + op* _op_sep, inventory[op].item_name, 0.5, 0.5, 0);
			//the arrow
			if item_pos == op{
				draw_sprite_ext(spr_textbox_arrow, 0, x_border + text_offset + option_x_offset + arrow_x_offset, option_y_offset + op* _op_sep, minimap_scale, minimap_scale, 0, c_white, 1 );	
			}
		}
