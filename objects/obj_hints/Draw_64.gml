if (global.in_hints) {
	...
	for (var row = 0; row < array_length(hint_grid); ++row) {
	    for (var col = 0; col < array_length(hint_grid[row]); ++col) {
			...
			draw_sprite_ext(spr_hint_box, -1, hint_grid_x + col * hint_grid_spacing, hint_grid_y + row * hint_grid_spacing, box_scale * ui_scale, box_scale * ui_scale, 0, c_white, 1);
			draw_sprite_ext(hint_sprite, -1, icon_padding + hint_grid_x + col * hint_grid_spacing, icon_padding + hint_grid_y + row * hint_grid_spacing, icon_scale * ui_scale, icon_scale * ui_scale, 0, c_white, 1);
		}
	}
