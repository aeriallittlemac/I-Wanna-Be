...
void main()
{
	...
	for(i=0; i < numEnabled; i++)
	{
		...
		//smooth attenuation
		vec2 delta = vec2(v_vPosition) - lightPos.xy;

		// squash vertically for isometric (tweak this)
		delta.y *= 2.0;   // bigger = flatter oval
		// delta.x *= 0.8; // optional horizontal tweak

		float attenuation = max(1.0 - length(delta)/range, 0.0);
